...
const initSales = () => {
  ...
  router.post("/", authMiddleware, async (req, res) => {
    try {
      const { body } = req;
      const uploaded = await postSale(body.salesData, body.salesItem);
  ...
  router.post("/docnum", authMiddleware, async (req, res) => {
    try {
      const { body } = req;
      const uploaded = await postSaleWDocnum(body.salesData, body.salesItem);
  ...
  router.put("/:docnum", authMiddleware, async (req, res) => {
    try {
      const { docnum } = req.params;
      const { body } = req;
      const updated = await putSale(docnum, body.salesData, body.salesItem, body.removedItem);

...
const getItem = async () => {
    try {
        ...
        const inJoin = await Promise.all(items.map(async (item) => {
            const itemClass = await models.itemfile.findOne({ 
                where: { itmcde: item.itmcde }
            });

            return {
                ...item.dataValues,
                deptdescription: itemClass ? itemClass.deptdescription : null 
            };
        }));
...
const postItemFile = async (body) => {
    try {
        const existingItem = await models.itemfile.findOne({ where: { itmcde: body.itmcde } });
...
const postItemFileDocnum = async (body) => {
    try {
        ...
        while (validate) {
            ...
            validate = await models.itemfile.findOne({ // Use models.itemfile instead of models.itmcde
                attributes: ['itmcde'],
                where: { itmcde: itmcde } 
            });
        }
...
const deleteItem = async (docnum) => {
    try {
        const deleted = await models.itemfile.destroy({ where: { itmcde: docnum } });

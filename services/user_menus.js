...
const getAndGroupUserMenus = async (usrcde) => {
    try {
        ...
        const mainWithSubmenus = mainMenus.map(menu => {
            ...
            if (menu.mensub) {
                subMenus = modifiedFindMenus.filter(subMenu => subMenu.mengrp === menu.mensub);
            }

            const subMenusWithSuperSub = subMenus.map(subMenu => {
                let superSubmenus = [];
                if (subMenu.mensub) {
                    superSubmenus = modifiedFindMenus.filter(superSubmenu => subMenu.mensub === superSubmenu.mengrp);
                }
...
const createUserMenus = async (body) => {
    try {
        if (Array.isArray(body)) {
            const createdUserMenus = await Promise.all(body.map(async (item) => {
